...
<template>
  <media-menu>
    ...
    <!-- Menu Items -->
    <media-menu-items
      :class="`${isMobile ? '!tw-video-bottom-[unset] !tw-video-left-[unset] !tw-video-right-20 !tw-video-top-1 !tw-video-w-full !tw-video-max-w-[260px] max-height-custom' : ''} ${isMobile && isHorizontalScreen ? '!tw-video-max-h-[260px]' : ''}`"
      class="tw-video-text-xs xsm:tw-video-text-sm !tw-video-z-20 tw-video-animate-out tw-video-fade-out tw-video-slide-out-to-bottom-2 data-[open]:tw-video-animate-in data-[open]:tw-video-fade-in data-[open]:tw-video-slide-in-from-bottom-4 tw-video-flex tw-video-h-[var(--menu-height)] tw-video-max-h-[260px] md:tw-video-max-h-[300px] lg:tw-video-max-h-[400px] tw-video-min-w-[240px] tw-video-flex-col tw-video-overflow-y-auto tw-video-overscroll-y-contain tw-video-rounded-md tw-video-border tw-video-border-white/10 tw-video-bg-black/95 tw-video-p-2.5 tw-video-font-sans tw-video-text-[15px] tw-video-font-medium tw-video-outline-none tw-video-backdrop-blur-sm tw-video-transition-[height] tw-video-duration-300 tw-video-will-change-[height] data-[resizing]:tw-video-overflow-hidden"
      :placement="placement"
    >
      <slot name="content" />
    </media-menu-items>
